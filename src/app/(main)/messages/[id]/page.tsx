...
export default function ChatRoomPage() {
  ...
  const formatMessageTime = (dateProp: any) => {
    if (!dateProp) return '';
    // handle firestore timestamp or raw Date object
    ...
    <div className="flex flex-col h-screen bg-[#F8F9FA] pb-safe">
      ...
      <header className="sticky top-0 z-40 flex items-center justify-between bg-white/90 p-3 backdrop-blur-md border-b border-[#F1F3F5] shadow-sm">
        <div className="flex items-center space-x-3">
          ...
          >
            {otherProfile?.avatarUrl || otherImage ? (
              <img src={otherProfile?.avatarUrl || otherImage} alt={otherProfile?.nickname || otherName} className="w-9 h-9 rounded-full object-cover border border-[#F1F3F5]" />
            ) : (
              <div className="w-9 h-9 rounded-full bg-[#E9ECEF]" />
            )}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-white">
        <div className="flex justify-center my-4">
          <span className="bg-[#F8F9FA] px-3 py-1 rounded-full text-[11px] font-bold text-[#ADB5BD] border border-[#F1F3F5]">
            2026년 3월 17일
          </span>
        </div>
            ...
            <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"} mb-4`}>
              <div className={`flex items-end max-w-[75%] ${isMe ? "flex-row-reverse space-x-reverse" : "space-x-2"}`}>
                ...
                {!isMe && (
                  ...
                  >
                    <img src={otherProfile?.avatarUrl || otherImage} alt={otherProfile?.nickname || otherName} className="w-7 h-7 rounded-full object-cover border border-[#F1F3F5] mb-1" />
                  </button>
                )}
                ...
                <div className="flex flex-col space-y-1 relative">
                  {/* Reply To Reference */}
                  {msg.replyTo && (
                    <div className={`text-[12px] opacity-70 mb-1 px-3 py-1.5 rounded-xl border ${
                      isMe ? "bg-white/10 border-white/20 text-white" : "bg-black/5 border-black/10 text-[#495057]"
                    } flex flex-col`}>
                  ...
                  <div className={`px-4 py-2.5 rounded-2xl shadow-sm max-w-[260px] relative group ${
                    isMe 
                    ...
                    <div className="flex flex-col space-y-3 pt-1">
                      ...
                      <button 
                        ...
                        className={`w-full mt-2 py-2.5 text-[13px] font-black rounded-xl shadow-sm transition-colors active:scale-95 ${
